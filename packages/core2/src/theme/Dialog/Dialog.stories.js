...
const generateParagraphs = n => {
  let items = [];
  for (let i = 0; i < n; i++) {
    items.push(<Paragraph sx={{ my: 36, color: "mono300" }} />);
  }
  return items;
};
  ...
  return (
    <Box>
      <Box sx={{ maxWidth: 600, p: "s9" }}>
        ...
        <Box as={"p"} sx={{ my: "s9" }}>
          Below we show responsive dialog. On desktop it's anchored to the
          button (bottom left), on mobile it's anchored to the window (slide
          from bottom).
        </Box>
